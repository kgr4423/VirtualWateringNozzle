...
public:
	...
	{
		if( wSize != wReqRxSize ){
			sprintf_s( Error, "Rx data size mismatch %d %d\r\n", wReqRxSize, wSize );
	...
	virtual BOOL Open( BYTE bModuleID )
	{
		pParentModule = NULL;
		bPort = 0;
		bID   = bModuleID;
	...
	virtual BOOL Open( ControlModule* pParent, BYTE bParentPort, BYTE bModuleID )
	{
		BYTE b;

		pParentModule = pParent;
		bPort = bParentPort;
		bID   = bModuleID;

		if( !Transceive( CM_OPEN, NULL, 0, &b, 1 ) ){
			sprintf_s( Error, "Unable to open ID=%d module\r\n", 0x00FF&bID );
			return( FALSE ); 
		}
		if( b != CM_OPEN  ){
			sprintf_s( Error, "Unexpected return value\r\n" );
			return( FALSE );
		}
		return( TRUE );
	}
	...
	virtual BOOL SetID( BYTE bNewID )
	{
		if( bNewID == CM_MASTER_ID ){
			sprintf_s( Error, "ID=255(CM_MASTER_ID) isn't permitted\r\n", 0x00FF&bID );
			return( FALSE ); 
		}

		if( !Transceive( CM_SET_ID, &bNewID, 1, NULL, 0 )) return( FALSE ); 
		bID = bNewID;
		return( TRUE );
	}
