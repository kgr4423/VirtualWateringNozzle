...
int Server::Connect() {
	...
	// TCPクライアントからの接続要求を待つ
	listen(sock0, 5);
	printf("Waiting Connection of Client\n");
	// TCPクライアントからの接続要求を受け付ける
	len = sizeof(client);
	sock = accept(sock0, (struct sockaddr*) & client, &len);
	printf("Successed at Connection\n");
...
int Server::Receive() {
	...
	printf("moterflag : %lf sec \n", receive_num);

	return 0;
}

// Unityにデータを送信
int Server::Send(CM01* pModule) {
	//送信したいデータをmessageに入れる
	if (pModule->bBoardID_PA == CM01_SENSOR_BOARD) {
